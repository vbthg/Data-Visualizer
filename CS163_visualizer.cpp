...
int main()
{
    sf::RenderWindow window(sf::VideoMode(800, 600), "NotchManager Sandbox");
    window.setFramerateLimit(60);

    GUI::NotchManager::getInstance().init(sf::Vector2f(800.f, 600.f));
...
const float DRAG_EXIT_DELAY = 0.15f; // Đợi 0.15 giây mới thực sự tắt Notch
    ...
    while(window.isOpen())
    {
        ...
        if (isDragging && !wasDragging) {
            GUI::NotchManager::getInstance().pushNotification(GUI::Scenario::FileTray, "Files Tray", "Drop a Text File", L"\uE4AA");
        }
